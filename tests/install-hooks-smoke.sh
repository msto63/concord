...
chk "install-hooks writes 10 files" "$o" "10 hook files"
chk "install-hooks exit 0" "$rc" 0

# Files present in <coord>/hooks/.
for f in lib.sh session-start.sh session-end.sh user-prompt.sh post-tool.sh pre-tool.sh statusline.sh install.sh uninstall.sh shared-regions; do
  [ -f "$COORD/hooks/$f" ] || { echo "✗ missing $f"; fail=1; }
done
[ "$fail" = 0 ] && echo "✓ all 10 hook files materialized"
